  ...
  return (
    <StyledDiv pt="10px" css={props}>
      <Flex flexDirection="column">
        <Text fontSize="1.3em">{dummyData.hashtag}</Text>
...
const TrendForYou = ({ data }) => {
  const rowData = data.reduce((rows, key, index: number) => {
    if (index % 2 == 0) {
      rows.push([key]);
    } else {
  ...
  return (
    ...
    >
      ...
      {rowData.map((item, index: number) => {
        return (
          <TrendingTileContainer
            title={"Trending Rank :: " + (index + 1)}
            data={item}
            key={index}
          />
        );
